...
function base64(options) {
  return this._updateFormatOut('base64');
}
  ...
  if (typeof callback === 'function') {
    ...
    if (this._isStreamInput()) {
      ...
      return new Promise((resolve, reject) => {
        this.once('finish', () => {
          this._flattenBufferIn();
          sharp.pipeline(this.options, (err, data, info, hist) => {
            if (err) {
              reject(err);
            } else {
              if (this.options.resolveWithObject) {
                if (this.options.plotHistogram)
                {
                  resolve({ data, info, hist });
                } else {
                  resolve({ data, info });
                }
              } else {
        ...
        sharp.pipeline(this.options, (err, data, info, hist) => {
          if (err) {
            ...
            if (this.options.resolveWithObject) {
              if (this.options.plotHistogram)
              {
                resolve({ data: data, info: info, hist: hist });
              }
              else {
