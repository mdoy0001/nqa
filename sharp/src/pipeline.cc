 ...
 public:
  ...
  void Execute() {
    ...
    try {
      ...
      bool const shouldAdjustTones = baton->shadows != 0.0 || baton->highlights != 0.0 || baton->midtones != 0.0;
      bool const shouldAdjustContrast = baton->contrast != 0.0 || baton->blackpoint != 0.0 || baton->recovery != 0.0;
      bool const shouldApplyTemperature = baton->temperature != 0.0;
      ...
      if (shouldAdjustContrast) {
        image = sharp::AdjustContrast(image, baton->contrast, baton->blackpoint, baton->recovery);
      }

      if (shouldAdjustTones) {
        image = sharp::AdjustTones(image, baton->shadows, baton->highlights, baton->midtones);
      }

      if (shouldApplyTemperature) {
        image = sharp::AdjustTemperature(image, baton->temperature);
      }
      ...
      if (baton->fileOut.empty()) {
        ...
        if (baton->formatOut == "jpeg" || (baton->formatOut == "input" && inputImageType == sharp::ImageType::JPEG)) {
          ...
          baton->formatOut = "raw";
        } else if (baton->formatOut == "base64") {
          // Write PNG to buffer
          sharp::AssertImageTypeDimensions(image, sharp::ImageType::PNG);
          VipsArea *area = reinterpret_cast<VipsArea*>(image.pngsave_buffer(VImage::option()
            ->set("strip", !baton->withMetadata)
            ...
            ->set("bitdepth", baton->pngBitdepth)
            ->set("dither", baton->pngDither)));
          const BYTE* buf = static_cast<BYTE*>(area->data);
          baton->bufferOutLength = area->length;
          area->free_fn = nullptr;
          vips_area_unref(area);

          baton->base64 = base64_encode(buf, baton->bufferOutLength);

          if (baton->plotHistogram) {
            baton->bufferOut = static_cast<char*>(sharp::PlotHistogram(image).write_to_memory(&baton->bufferOutLength));
            if (baton->bufferOut == nullptr) {
              (baton->err).append("Could not allocate enough memory for histogram output");
              return Error();
            }
          }

          baton->formatOut = "base64";
        } else {
      ...
      if (baton->bufferOutLength > 0) {
        if (baton->formatOut == "base64") {
          ...
          Napi::Buffer<char> data = Napi::Buffer<char>::New(env, static_cast<char*>(baton->bufferOut),
           ...
           Callback().MakeCallback(Receiver().Value(), { env.Null(), Napi::String::New(env, baton->base64), info, data });
        } else {
          // Add buffer size to info
          info.Set("size", static_cast<uint32_t>(baton->bufferOutLength));
          // Pass ownership of output data to Buffer instance
...
Napi::Value pipeline(const Napi::CallbackInfo& info) {
  ...
  baton->contrast = sharp::AttrAsDouble(options, "contrast");
  baton->blackpoint = sharp::AttrAsDouble(options, "blackpoint");
  baton->recovery = sharp::AttrAsDouble(options, "recovery");

  baton->temperature = sharp::AttrAsDouble(options, "temperature");


  baton->shadows = sharp::AttrAsDouble(options, "shadows");
  baton->highlights = sharp::AttrAsDouble(options, "highlights");
  baton->midtones = sharp::AttrAsDouble(options, "midtones");

  baton->plotHistogram = sharp::AttrAsBool(options, "plotHistogram");

  baton->base64 = sharp::AttrAsStr(options, "base64");
